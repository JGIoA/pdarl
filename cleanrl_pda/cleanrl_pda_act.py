# Single-file implementation of PDA_ACT in the CleanRL style.
...
class Args:
    ...
    """the learning rate of the SOAP optimizers (V, SumAdv, Actor)"""
    num_envs: int = 10
    ...
    """toggle learning rate annealing for V/SumAdv/Actor optimizers"""
    gamma: float = 0.99
    ...
    """the K epochs to update each of V, SumAdv, Actor per iteration"""
    ...
    act_noise: float = 1.3
    """action noise magnitude for exploration"""
...
class ActorNetwork(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.network = _build_mlp(obs_dim, act_dim, hidden_sizes, activation)
...
class Agent(nn.Module):
    def __init__(
        ...
        assert isinstance(act_space, gym.spaces.Box), (
            "PDA_ACT only supports continuous (Box) action spaces"
        )
        ...
        self.actor_model = ActorNetwork(
            self.obs_dim, self.act_dim, self.act_lims, args.hidden_sizes, activation
        )
        ...
        self.optimActor = SOAP(self.actor_model.parameters(), lr=args.learning_rate)
    ...
    def compute_act(self, obs) -> torch.Tensor:
        """Greedy actor output in ``[-1, 1]``."""
        return self.actor_model(self._obs_to_tensor(obs))

    def exploration_noise(self, act: torch.Tensor) -> torch.Tensor:
        """Add Gaussian noise (decayed by ``beta**-0.3``) and clamp to ``[-1, 1]``."""
        sigma = max(self.act_noise / (self.beta**0.3), 0.0)
        return torch.clamp(act + torch.randn_like(act) * sigma, -1.0, 1.0)

    def action_to_env(self, action: torch.Tensor) -> torch.Tensor:
        """Map a normalized action in ``[-1, 1]`` to the env action range."""
        return self.actor_model.map_action(action)
    ...
    def compute_returns(self, rd: dict) -> tuple[torch.Tensor, torch.Tensor]:
        """GAE advantages and (optionally rms-scaled) returns from a rollout dict."""
    ...
    def optimize_actor(
        ...
        for _step in range(repeat):
            ...
            for start in range(0, total_size, split_batch_size):
                ...
                current_a = self.actor_model(mb_obs)
                regularization = (
                    beta_pow * self.reg * torch.sum(current_a**2, dim=1, keepdim=True)
                ) / self.sum_beta
                actor_loss = (-self.SumAdv(mb_obs, current_a) + regularization).mean()
    ...
    def update_policy(
        ...
        if self.args.anneal_lr:
            ...
            for opt in (self.optimV, self.optimSumAdv, self.optimActor):
...
def evaluate(
    ...
    try:
        ...
        episode_count = 0
        is_box = isinstance(test_envs.single_action_space, gym.spaces.Box)

        while episode_count < num_episodes:
            with torch.no_grad():
                action = agent.compute_act(next_obs).detach()
            env_action = agent.action_to_env(action) if is_box else action
...
if __name__ == "__main__":
    ...
    seeds_train = [
        int(rng.integers(0, 2**31, dtype=np.int32)) for _ in range(args.num_envs)
    ]
    seeds_test = [
        int(rng.integers(0, 2**31, dtype=np.int32)) for _ in range(args.num_test_envs)
    ]
    ...
    # Read true action bounds from a one-shot raw env
    _probe_env = gym.make(args.env_id, **(args.env_kargs or {}))
    assert isinstance(_probe_env.action_space, gym.spaces.Box), (
        "only continuous action space is supported"
    )
    act_lims = [
        float(_probe_env.action_space.low[0]),
        float(_probe_env.action_space.high[0]),
    ]
    _probe_env.close()
    ...
    agent = Agent(envs, args, device, act_lims=act_lims).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            terminated[step] = torch.as_tensor(
